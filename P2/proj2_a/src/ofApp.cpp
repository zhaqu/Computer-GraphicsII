...
void ofApp::setup(){
	...
	mesh.load("shield.ply");
	shader.load("vertex_color.vert", "vertex_color.frag");
	diffuse_Tex.load("shield_diffuse.png");
	normal_Tex.load("shield_normal.png");
	specular_Tex.load("shield_spec.png");
	//mesh.enableColors();
	auto vertices_num = mesh.getVertices();
	//auto faces = mesh.getUniqueFaces();
	//calculate the bound and center
	const glm::vec3* vertices = mesh.getVerticesPointer();
	const glm::vec2* uvs = mesh.getTexCoordsPointer();
	const unsigned* triangles = mesh.getIndexPointer();
	const auto tri_count = mesh.getIndices();
	glm::vec3 Tangent;
	for (unsigned i = 0; i < tri_count.size()-2; i+=3)
	{
		const glm::vec3& v0 = vertices[triangles[i]];
		const glm::vec3& v1 = vertices[triangles[i + 1]];
		const glm::vec3& v2 = vertices[triangles[i + 2]];
		const glm::vec2& uv0 = uvs[triangles[i]];
		const glm::vec2& uv1 = uvs[triangles[i+1]];
		const glm::vec2& uv2 = uvs[triangles[i+2]];
		glm::vec3 edge_1 = glm::vec3(v1.x - v0.x, v1.y - v0.y, v1.z - v0.z);
		glm::vec3 edge_2 = glm::vec3(v2.x - v0.x, v2.y - v0.y, v2.z - v0.z);
		float delta_u1 = uv1.x - uv0.x;
		float delta_v1 = uv1.y - uv0.y;
		float delta_u2 = uv2.x - uv0.x;
		float delta_v2 = uv2.y - uv0.y;
		float f = 1.0 / (delta_u1 * delta_v2 - delta_u2 * delta_v1);
	...
	projection = glm::perspective(glm::radians(45.0f), (float)ofGetHeight() / (float)ofGetWidth(), 0.1f, 1000.0f);
...
void ofApp::draw(){
	...
	shader.begin();
	shader.setUniformTexture("diffuse_Tex", diffuse_Tex, 0);
	shader.setUniformTexture("normal_Tex", normal_Tex, 1);
	shader.setUniformTexture("specular_Tex", specular_Tex, 2);
